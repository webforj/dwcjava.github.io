...
Once webforJ and its dependencies are set up in your project, you're ready to create the app structure. This article will walk through the key elements of a basic webforJ app, specifically focusing on the `Application` and `HomeView` classes, which are the foundational classes in the `hello-world` project.
...
- `@InlineStyleSheet`: Links an external CSS file, styles.css, allowing custom styling for the app.
...
`HomeView` extends `Composite<FlexLayout>`, which allows it to act as a reusable component containing a layout. The `@InlineStyleSheet` annotation applies a CSS stylesheet, and `@Route("/")` makes this the root route of the app.
...
- Layout Settings: The FlexLayout is configured to stack items vertically, using `FlexDirection.COLUMN`, center them using `FlexJustifyContent.CENTER and FlexAlignment.CENTER`, and add spacing by calling `setSpacing("3em")`.
...
The `styles.css` file provides custom styling for your webforJ app. This CSS file is referenced in the HomeView class using the `@InlineStyleSheet` annotation, which allows the app to apply styles to components within the app.
